...
let itemsRef = db.ref( '/root/items/AJava' );
      ...
      handleSubmit() {
       ...
       this.setState({item : this.props.items,val : this.props.items[this.state.index].name})
        ...
        if(this.state.index <= this.props.items.length-1){
                    db.ref('/root/answers/AJava/'+this.props.items[this.state.index].name).push().set(
...
export default class Ajava extends Component {
  constructor(){
    super()
    this.state = {
        name : '',
