...
import {
  ...
  Button,
  FlatList,
  AlertAndroid,ImageBackground,Dimensions
...
let itemsRef = db.ref( '/root/items/biology' );
      ...
      handleSubmit() {
       ...
       this.setState({item : this.props.items,val : this.props.items[this.state.index].name})
        ...
        if(this.state.index <= this.props.items.length-1){
                    db.ref('/root/answers/biology/'+this.props.items[this.state.index].name).push().set(
